...
router.post('/', function (req, res, next) {
    ...
    sql.connect(db, function (err) {
        ...
        request.query(util.format(qryString, phone, CardNumber), function (err, recordset) {
            ...
            if (recordset.recordset.length > 0) {
                ...
                res.send({
                    status: false,
                    "errmessage": "شماره تلفن همراه با شماره کارت ثبت شده در سامانه مطابقت ندارد. لطفا با واحد پشتیبانی تماس حاصل نمایید"
                });
                res.end();
                sql.close();
                return;
