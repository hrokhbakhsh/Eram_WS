...
router.post('/', function (req, res, next) {
    ...
    var qryStr = "SET TRANSACTION ISOLATION LEVEL SERIALIZABLE; " +
        "BEGIN TRANSACTION; " +
        "IF EXISTS (SELECT 1 FROM [app].[MobileDeviceInfo] WHERE PhoneNumber = \'%s\') " +
        "BEGIN " +
        ...
        "ELSE " +
        "BEGIN " +
        "INSERT INTO [app].[MobileDeviceInfo] (PhoneNumber, PersonID, TokenKey, CreatedDateTime) " +
        "VALUES (\'%s\', \'%s\', \'%s\', \'%s\') " +
        "END " +
        "COMMIT TRANSACTION; ";
    ...
    sql.connect(db, function (err) {
        ...
        request.query(util.format(qryStr, PhoneNumber, PersonID, TokenKey, formatted, PhoneNumber, PhoneNumber, PersonID, TokenKey, formatted), function (err, recordset) {
